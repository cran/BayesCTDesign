...
plot_table.bayes_ctd_array <- function(bayes_ctd_array = NULL, measure = "power", tab_type = "WX|YZ", smooth = FALSE,
    ...
    if (tab_type == "WX|YZ") {
        ...
        if (length(bayes_ctd_array$subj_per_arm) < 2)
            stop("plot_table() requires x-axis variable two have at least two elements.")
        ...
        if (smooth == TRUE) {
            if (length(bayes_ctd_array$subj_per_arm) < 3){
                stop("x-axis variable must have at least 3 elements or smoothing crashes.")
            }
            if (length(bayes_ctd_array$subj_per_arm) < 7){
                warning("x-axis variable may have too few values for smoothing.")
                message("x-axis variable may have too few values for smoothing.")
            }
    ...
    if (tab_type == "WY|XZ") {
        ...
        table_mat <- print_table(bayes_ctd_array = bayes_ctd_array, measure = measure, tab_type = "WY|XZ", a0_val = a0_val,
            rand_control_diff_val = rand_control_diff_val, print_chg_warn = 0)
        negchk <- 1*(as.numeric(bayes_ctd_array$effect_vals) < 0)
        if (is.matrix(table_mat)){
          colvals <- as.numeric(colnames(table_mat))
        }
        if (sum(negchk)>0 & length(negchk) > 1){
          for (l in 1:length(negchk)){
            if (negchk[l] == 1) colnames(table_mat)[l] <- paste('N',-1*colvals[l], sep="")
          }
        }
        ...
        if (plot_out == TRUE) {
            ...
            #Construct proper values for the line variable.
            avec <- rep(NA,dim(table_dat)[1])
            for (i in 1:dim(table_dat)[1]) {
                temp1 <- unlist(strsplit(as.character(table_dat[i, 2]), split = c("X"), fixed = TRUE))
                temp2 <- unlist(strsplit(as.character(table_dat[i, 2]), split = c("N"), fixed = TRUE))
                if (length(temp1) == 2){
                  avec[i] <- as.numeric(temp1)[2]
                }
                if (length(temp2) == 2){
                  avec[i] <- -1*as.numeric(temp2)[2]
                }
            }
    ...
    if (tab_type == "WZ|XY") {
        ...
        negchk <- 1*(as.numeric(bayes_ctd_array$rand_control_diff) < 0)
        if (is.matrix(table_mat)){
            colvals <- as.numeric(colnames(table_mat))
        }
        if (sum(negchk)>0 & length(negchk) > 1){
    ...
    if (tab_type == "XY|WZ") {
        ...
        if (length(bayes_ctd_array$a0_vals) < 2)
            stop("plot_table() requires x-axis variable two have at least two elements.")
        ...
        # Use a call to stats::loess to smooth simulation results is requested.
        if (smooth == TRUE) {
            if (length(bayes_ctd_array$a0_vals) < 3){
                stop("x-axis variable must have at least 3 elements or smoothing crashes.")
            }
            if (length(bayes_ctd_array$a0_vals) < 7){
                warning("x-axis variable may have too few values for smoothing.")
                message("x-axis variable may have too few values for smoothing.")
            }
            colcnt <- dim(table_mat)[2]
    ...
    if (tab_type == "YZ|WX") {
        ...
        if (length(bayes_ctd_array$effect_vals) < 2)
            stop("plot_table() requires x-axis variable two have at least two elements.")
        ...
        # Use a call to stats::loess to smooth simulation results is requested.
        if (smooth == TRUE) {
            if (length(bayes_ctd_array$effect_vals) < 3){
                stop("x-axis variable must have at least 3 elements or smoothing crashes.")
            }
            if (length(bayes_ctd_array$effect_vals) < 7){
                warning("x-axis variable may have too few values for smoothing.")
                message("x-axis variable may have too few values for smoothing.")
            }
            colcnt <- dim(table_mat)[2]
    ...
    if (tab_type == "ZX|WY") {
      ...
      if (length(bayes_ctd_array$rand_control_diff) < 2)
          stop("plot_table() requires x-axis variable two have at least two elements.")
      ...
      # Use a call to stats::loess to smooth simulation results is requested.
      if (smooth == TRUE) {
        if (length(bayes_ctd_array$rand_control_diff) < 3){
          stop("x-axis variable must have at least 3 elements or smoothing crashes.")
        }
        if (length(bayes_ctd_array$rand_control_diff) < 7){
            warning("x-axis variable may have too few values for smoothing.")
            message("x-axis variable may have too few values for smoothing.")
        }
        colcnt <- dim(table_mat)[2]
...
plot.bayes_ctd_array <- function(x = NULL, measure = "power", tab_type = "WX|YZ", smooth = FALSE,
  ...
  if (tab_type == "WX|YZ") {
      ...
      if (length(x$subj_per_arm) < 2)
          stop("plot() requires x-axis variable two have at least two elements.")
      # Line Plot of Design Characteristics: Sample Size by a0
      ...
      # Use a call to stats::loess to smooth simulation results is requested.
      if (smooth == TRUE) {
          if (length(x$subj_per_arm) < 3){
              stop("x-axis variable must have at least 3 elements or smoothing crashes.")
          }
          if (length(x$subj_per_arm) < 7){
              warning("x-axis variable may have too few values for smoothing.")
              message("x-axis variable may have too few values for smoothing.")
          }
          colcnt <- dim(table_mat)[2]
  ...
  if (tab_type == "WY|XZ") {
      ...
      table_mat <- print(x = x, measure = measure, tab_type = "WY|XZ", a0_val = a0_val,
                               rand_control_diff_val = rand_control_diff_val, print_chg_warn = 0)
      negchk <- 1*(as.numeric(x$effect_vals) < 0)
      if (is.matrix(table_mat)){
          colvals <- as.numeric(colnames(table_mat))
      }
      if (sum(negchk)>0 & length(negchk) > 1){
  ...
  if (tab_type == "WZ|XY") {
      ...
      negchk <- 1*(as.numeric(x$rand_control_diff) < 0)
      if (is.matrix(table_mat)){
          colvals <- as.numeric(colnames(table_mat))
      }
      if (sum(negchk)>0 & length(negchk) > 1){
  ...
  if (tab_type == "XY|WZ") {
      ...
      # Use a call to stats::loess to smooth simulation results is requested.
      if (smooth == TRUE) {
          if (length(x$a0_vals) < 3){
              stop("x-axis variable must have at least 3 elements or smoothing crashes.")
          }
          if (length(x$a0_vals) < 7){
              warning("x-axis variable may have too few values for smoothing.")
              message("x-axis variable may have too few values for smoothing.")
          }
          colcnt <- dim(table_mat)[2]
  ...
  if (tab_type == "YZ|WX") {
      ...
      # Use a call to stats::loess to smooth simulation results is requested.
      if (smooth == TRUE) {
          if (length(x$effect_vals) < 3){
              stop("x-axis variable must have at least 3 elements or smoothing crashes.")
          }
          if (length(x$effect_vals) < 7){
              warning("x-axis variable may have too few values for smoothing.")
              message("x-axis variable may have too few values for smoothing.")
          }
          colcnt <- dim(table_mat)[2]
  ...
  if (tab_type == "ZX|WY") {
      ...
      if (length(x$rand_control_diff) < 2)
          stop("plot() requires x-axis variable two have at least two elements.")
      # Line Plot of Design Characteristics: Control Diffferences by a0
      # First generate the table using a call to print.
      ...
      # Use a call to stats::loess to smooth simulation results is requested.
      if (smooth == TRUE) {
          if (length(x$rand_control_diff) < 3){
              stop("x-axis variable must have at least 3 elements or smoothing crashes.")
          }
          if (length(x$rand_control_diff) < 7){
              warning("x-axis variable may have too few values for smoothing.")
              message("x-axis variable may have too few values for smoothing.")
          }
          colcnt <- dim(table_mat)[2]
